...
d_tipo_taxi,
d_mes,
 sum(c_monto_tarifa) as mt_monto_tarifa,
  COUNT(*) AS casos
FROM (
  SELECT
    (
    ...
    LIMIT
      1 ) AS prob_clase_1,
    tgt_propina_sup_20, d_forma_pago,d_punto_ab,c_monto_tarifa,d_tipo_taxi,
    d_mes
  FROM
    ML.PREDICT( MODEL `[PROJECT_ID].[DATASET].modelo_proba_propina`,
      (
      SELECT
        *
              FROM
